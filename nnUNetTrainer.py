...
from nnunetv2.training.loss.Dice import BinaryDiceLoss
...
from nnunetv2.training.nnUNetTrainer.variants.network_architecture.SBMANet import SBMANet
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.num_iterations_per_epoch = 140
        self.num_val_iterations_per_epoch = 35
        self.num_epochs = 800
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.network = SBMANet(1,2).to(self.device)
    ...
    def train_step(self, batch: dict) -> dict:
        data = batch['data']       #2,3,16,192,192

        target = batch['target']                 #2,1,16,192,192    list = 7
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            # output = self.network(data1, data2, data3)     #2,1,512,512
            output = self.network(data)     #2,2,16,192,192
            # del data
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        print('Epoch: ', self.current_epoch, ' train Mean losses: ', loss_here)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        target = batch['target']  # 2,1,64,192,192
        ...
        # So autocast will only be active if we have a cuda device.
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            # output = self.network(data1, data2, data3)
    ...
    def on_epoch_end(self):
        ...
        # epoch 大于500后每隔 50 epoch 保存一次模型
        if (current_epoch + 1) % 50 == 0 and (current_epoch + 1) > 500 and current_epoch != (self.num_epochs - 1):
            self.save_checkpoint(join(self.output_folder, f'Epoch_{current_epoch}_checkpoint.pth'))
    ...
    def run_training(self):
        self.on_train_start()                                   #准备数据

        for epoch in range(self.current_epoch, self.num_epochs):
            self.on_epoch_start()                               #记录epoch开始时间

            self.on_train_epoch_start()                         #开启训练模式，学习率记录
            train_outputs = []
            for batch_id in range(self.num_iterations_per_epoch):
                train_outputs.append(self.train_step(next(self.dataloader_train)))
                print('Epoch: ', epoch, ' batch_id: ', batch_id, ' losses: ', train_outputs[-1]['loss'])
            self.on_train_epoch_end(train_outputs)

            with torch.no_grad():
                self.on_validation_epoch_start()   #开启验证模式
                val_outputs = []
                for batch_id in range(self.num_val_iterations_per_epoch):
                    val_outputs.append(self.validation_step(next(self.dataloader_val)))
                    print('batch_id: ', batch_id, ' losses: ', val_outputs[-1]['loss'], ' tp_hard: ', val_outputs[-1]['tp_hard'], ' fp_hard: ', val_outputs[-1]['fp_hard'], ' fn_hard: ', val_outputs[-1]['fn_hard'])
